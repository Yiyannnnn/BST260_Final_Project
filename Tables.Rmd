...
OTU_H <- OTU |> dplyr::select(sampleMetadata$sample_id) |> 
  t() |> as.data.frame()

OTU_IBD <- OTU |>
  dplyr::select(sampleMetadata_disease[which(sampleMetadata_disease$study_condition == "IBD"),]$sample_id) |>
    t() |> as.data.frame()

OTU_CRC <- OTU |>
  dplyr::select(sampleMetadata_disease[which(sampleMetadata_disease$study_condition == "CRC"),]$sample_id) |> 
  t() |> as.data.frame()

OTU_IGT <- OTU |>
  dplyr::select(sampleMetadata_disease[which(sampleMetadata_disease$study_condition == "IGT"),]$sample_id) |> 
  t() |> as.data.frame()

OTU_T2D <- OTU |>
  dplyr::select(sampleMetadata_disease[which(sampleMetadata_disease$study_condition == "T2D"),]$sample_id) |>
  t() |> as.data.frame() 
```


### Table 1. EDA summary table for age and gender information for all study samples
...
### Figure 1. Alpha diversity for each health status
...
### Figure 2. PCA plot for Healthy vs IBD, Healthy vs CRC, Healthy vs IGT, and Healthy vs T2D
...
```{r,eval=FALSE,include=FALSE,message=FALSE,echo=FALSE}
library(caTools)
library(randomForest)
library(PRROC)
library(data.table)
...
```{r,echo=FALSE,message=FALSE}
fit_rf_IBD <- readRDS("RF_IBD.rds")
fit_rf_CRC <- readRDS("RF_CRC.rds")
fit_rf_IGT <- readRDS("RF_IGT.rds")
fit_rf_T2D <- readRDS("RF_T2D.rds")
...
### Figure 3. Venn diagram for the selected predictors of each disease classification
...
### Figure 4. Variable Importance Plot for Random Forest model of each disease classification
...
### Figure 5. Performance evalutation of models by Accuracy
...
### Figure 6. Performance evalutation of models by F1 Score
...
### Figure 7. Performance evalutation of models by AUPRC